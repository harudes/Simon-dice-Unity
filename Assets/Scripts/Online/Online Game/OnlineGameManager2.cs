...
public class OnlineGameManager2 : MonoBehaviourPunCallbacks
{
    public static OnlineGameManager2 instance;

    #region Private Fields

    [SerializeField]
    TMP_Text orderText, scoreText, lifesText, startCountdownText;
    ...
    OnlineVoiceController2 voiceController;
    ImageController imagesController;

    float markerTime = 8.0f, voiceTime = 10.0f;
    int playersReady = 0;
    ...
    public void Awake()
    {
        ...
        voiceController = GetComponent<OnlineVoiceController2>();
        imagesController = GetComponent<ImageController>();
    }
    ...
    void ReceiveSharedOrder(int type, bool isSimon, string text, string[] players, int color, int direction, string word)
    {
        ...
        if (type == 0)
        {
            imagesController.MovementOrder(color, isSimon, orderPlayers, direction, waitTime);
        }
        else
        {
    ...
    void GenerateIndividualOrder()
    {
        ...
        if (individualOrder.type == 0)
        {
            imagesController.MovementOrder(individualOrder.color, individualOrder.isSimon, individualOrder.players, individualOrder.direction, waitTime);
        }
        else
        {
    ...
    {
        string uri = "https://us-central1-simon-dice-76365.cloudfunctions.net/addGame?score=" + score;
